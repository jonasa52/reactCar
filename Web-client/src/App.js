        ...
        var linkTable = cars.map((car) => {
            return (
                <tr scope="row" key={car.regno}>
                    ...
                    <td onclick={this.props.saved}><Link to="/compare" class="btn btn-success">Compare</Link></td>
                </tr>
            )
        });
...
class Compare extends Component {
    ...
    const  carIndex = this.dataFromServer.findIndex( c =>{
       return c.id === id;
    });
    
    const car = {
        ...this.dataFromServer[carIndex]
    };

    const cars = [...this.dataFromServer];
    cars[carIndex] = car;
   ...
   // car.id = event.target.value;
    ...
    render()
   
        {
          ...
          var carss = this.state.dataFromServer;
          var compares = carss.map((car) => {
             // {car.name}

          
              
              <div>saved={() => this.handleCompareSave(car.id)}</div>
          })
